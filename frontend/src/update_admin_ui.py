    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 min-h-[70vh]">
            <div className="w-full md:w-64 glass-card p-4 flex flex-col gap-2 border border-slate-200 dark:border-white/10 shadow-sm">
                <button onClick={() => setActiveTab('projects')} className={`p-3 rounded-lg text-left font-medium transition-colors ${activeTab === 'projects' ? 'bg-violet-600 text-white shadow-md shadow-violet-500/20' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'}`}>Projects</button>
                ...
                <button onClick={() => setActiveTab('skills')} className={`p-3 rounded-lg text-left font-medium transition-colors ${activeTab === 'skills' ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'}`}>Skills</button>
                <button onClick={() => setActiveTab('profile')} className={`p-3 rounded-lg text-left font-medium transition-colors ${activeTab === 'profile' ? 'bg-fuchsia-600 text-white shadow-md shadow-fuchsia-500/20' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5'}`}>Profile & Resume</button>
...
function ProjectManager() {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold mb-6 text-slate-800 dark:text-white border-b border-slate-200 dark:border-white/10 pb-2">Add Project</h3>
            <form onSubmit={createProject} className="space-y-4 mb-10 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <button type="submit" className="bg-violet-600 hover:bg-violet-700 text-white font-semibold px-6 py-3 rounded-lg transition-colors shadow-md shadow-violet-500/20">Save Project</button>
            </form>
...
function CertificationManager() {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold mb-6 text-slate-800 dark:text-white border-b border-slate-200 dark:border-white/10 pb-2">Add Certification</h3>
            <form onSubmit={handleSubmit} className="space-y-4 mb-10 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-3 rounded-lg shadow-md shadow-emerald-500/20">Save Certification</button>
            </form>
...
function SkillManager() {
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold mb-6 text-slate-800 dark:text-white border-b border-slate-200 dark:border-white/10 pb-2">Manage Skills</h3>
            <form onSubmit={handleSubmit} className="space-y-4 mb-10 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <input type="range" min="0" max="100" value={form.percentage} onChange={e=>setForm({...form, percentage: e.target.value})} className="w-full accent-blue-600" required/>
                </div>
                <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg shadow-md shadow-blue-500/20 mt-4 block">Add Skill</button>
            </form>
...
function ProfileManager() {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
            ...
            <button type="submit" className="w-full md:w-auto mt-6 bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-semibold px-8 py-3 rounded-lg shadow-md shadow-fuchsia-500/20 transition-colors">Save Profile</button>
        </form>
    );
}
                    ...
                    <div key={m.id} className="p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
                            <div>
                                ...
                                <a href={`mailto:${m.email}`} className="text-violet-600 dark:text-violet-400 text-sm font-medium hover:underline">{m.email}</a>
                            </div>
...
with open(os.path.join(base_dir, "pages/AdminDashboard.jsx"), "w", encoding="utf-8") as f:
    f.write(content)

print("AdminDashboard UI updated successfully.")
